...
export default function CVDownloadButton({ cvParsed, cvKitPurchased, cvTier }: { cvParsed: boolean; cvKitPurchased?: boolean; cvTier?: string | null }) {
  ...
  return (
    <div className="rounded-2xl p-5"
      style={{ background: 'linear-gradient(#16161F, #16161F) padding-box, linear-gradient(135deg, rgba(106,168,245,0.35), rgba(79,143,232,0.25)) border-box', border: '1px solid transparent', boxShadow: '0 1px 2px rgba(0,0,0,0.45), 0 16px 40px rgba(0,0,0,0.35)' }}>
      ...
      {hasAccess ? (
        <>
          ...
          <Link href="/cv-ready"
            className="block text-center bg-gradient-to-r from-[#6AA8F5] to-[#4F8FE8] text-white text-xs font-black py-2.5 rounded-xl hover:opacity-90 transition-opacity">
            Open CV Kit →
          </Link>
        </>
      ) : cvParsed ? (
        <>
          ...
          <div className="space-y-2">
            ...
            <button
              onClick={handleBuy}
              disabled={loading}
              className="w-full bg-gradient-to-r from-[#6AA8F5] to-[#4F8FE8] text-white text-xs font-black py-2.5 rounded-xl hover:opacity-90 transition-opacity disabled:opacity-50">
