...
export default function ShapiCharacter({ mood = 'idle', size = 80, className = '', tint, face = true }: ShapiCharacterProps) {
  ...
  // Default glow is a soft, subtle steel-blue (no neon).
  const defaultGlow: Record<Mood, string> = {
    idle: 'rgba(139,176,224,0.38)',
    thinking: 'rgba(139,176,224,0.46)',
    happy: 'rgba(166,194,232,0.48)',
    listening: 'rgba(139,176,224,0.38)',
  }
  ...
  // Default fill is a subtle steel-blue gradient that very gently shifts — soft,
  // not neon. A tint, when passed, stays static.
  const animated = !tint
  const stops = tint
    ? [{ o: '0%', c: tint.from }, { o: '100%', c: tint.to }]
    : [
        { o: '0%', c: '#A6C2E8', vals: '#A6C2E8;#8BB0E0;#C2D3EE;#A6C2E8' },
        { o: '50%', c: '#8BB0E0', vals: '#8BB0E0;#C2D3EE;#A6C2E8;#8BB0E0' },
        { o: '100%', c: '#C2D3EE', vals: '#C2D3EE;#A6C2E8;#8BB0E0;#C2D3EE' },
      ]
    ...
    <div className={`relative inline-flex items-center justify-center ${className}`} style={{ width: size, height: size }}>
      ...
      <svg width={size} height={size} viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ position: 'relative' }}>
        ...
        <g style={{ animation: `shapiSpin ${ringSpin}s linear infinite`, transformOrigin: '50px 50px' }}>
          ...
          <circle cx="50" cy="5" r="2.2" fill={tint?.from || '#C2D3EE'} />
        </g>
        ...
        {mood === 'happy' && (
          <g style={{ animation: 'shapiSpinR 6s linear infinite', transformOrigin: '50px 50px' }}>
            <path d="M50 12 L52 18 L58 20 L52 22 L50 28 L48 22 L42 20 L48 18 Z" fill={tint?.from || '#C2D3EE'} />
          </g>
        )}
      </svg>
    </div>
