                  ...
                  <div key={i} className="flex items-center gap-2">
                    ...
                    >
                      {PROFICIENCY_LEVELS.map(lvl => (
                        <option key={lvl} value={lvl} style={{ background: '#0D0C14' }}>{lvl}</option>
                      ))}
                    </select>
                    <button
                      type="button"
                      onClick={() => setLanguagesSpoken(prev => prev.filter((_, j) => j !== i))}
                      className="text-[#7E7E8E] hover:text-[#FB7185] text-lg px-2"
              ...
              <div className="space-y-2">
                {rightToWork.map((r, i) => (
                  <div key={i} className="flex items-center gap-2">
                    ...
                    >
                      {RTW_BASIS.map(b => (
                        <option key={b.value} value={b.value} style={{ background: '#0D0C14' }}>{b.label}</option>
                      ))}
                    </select>
                    <button type="button" onClick={() => removeRtw(i)} className="text-[#7E7E8E] hover:text-[#FB7185] text-lg px-2" aria-label="Remove">✕</button>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div>
              ...
              <select className="field" value={salPeriod} onChange={e => setSalPeriod(e.target.value as 'month' | 'year')}>
                <option value="month" style={{ background: '#0D0C14' }}>Month</option>
                <option value="year" style={{ background: '#0D0C14' }}>Year</option>
              </select>
            </div>
                ...
                {pivotBands.map((p, i) => (
                  <div key={i} className="rounded-xl p-3" style={{ background: 'rgba(14,14,26,0.04)', border: '1px solid rgba(14,14,26,0.08)' }}>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button type="button" onClick={() => removePivot(i)} className="text-[#7E7E8E] hover:text-[#FB7185] text-sm">✕</button>
                    </div>
