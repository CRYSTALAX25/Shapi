...
export default async function ProfilePage() {
  ...
  const tierMeta: Record<string, { label: string; color: string; bg: string; emoji: string; description: string }> = {
    basic: { label: 'Basic Verified', color: '#9D8CFF', bg: 'rgba(157, 140, 255, 0.10)', emoji: '🔵', description: '1 of 2 reference chains complete' },
    strong: { label: 'Strongly Verified', color: '#9D8CFF', bg: 'rgba(157, 140, 255, 0.10)', emoji: '🟢', description: 'Both reference chains + peer reference complete' },
    premium: { label: 'Premium Verified', color: '#9D8CFF', bg: 'rgba(157, 140, 255, 0.10)', emoji: '🟡', description: 'Strong + AI cross-check passed with no conflicts' },
  }
    ...
    <aside className="lg:sticky lg:top-6 space-y-4 self-start">
      <div className="gradient-border-card rounded-2xl p-6">
        <div className="flex items-center gap-4 mb-4">
          {profile.profile_image_url ? (
            <img src={profile.profile_image_url as string} alt={(profile.full_name as string) || 'Profile'}
              className="w-16 h-16 rounded-full object-cover flex-shrink-0" style={{ border: '2px solid rgba(157, 140, 255, 0.3)' }} />
          ) : (
            <div className="w-16 h-16 rounded-full flex-shrink-0 flex items-center justify-center text-2xl font-black text-[#060609]" style={{ background: 'linear-gradient(135deg,#9D8CFF, #34D399)' }}>
              {((profile.full_name as string) || '?').charAt(0).toUpperCase()}
            </div>
          )}
          <div className="min-w-0">
            <h1 className="text-xl font-black leading-tight" style={{ color: '#9D8CFF' }}>{profile.full_name || 'Your Name'}</h1>
        ...
        <div className="flex items-center gap-3 mb-3">
          <div className="text-3xl font-black" style={{ background: 'linear-gradient(135deg, #9D8CFF, #34D399)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>{completion}%</div>
          <div className="flex-1">
            <div className="h-1.5 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.07)' }}>
              <div className="h-full rounded-full" style={{ width: `${completion}%`, background: 'linear-gradient(90deg, #9D8CFF, #34D399)' }} />
            </div>
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          {isLive && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-[#9D8CFF]/15 text-[#9D8CFF]">✓ Live</span>}
          ...
          {((profile.ai_resilience_score as number | null) ?? 0) >= 7 && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-[#9D8CFF]/15 text-[#9D8CFF]">🛡️ AI-Proof Asset</span>}
          {profile.ai_tier && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-[#9D8CFF]/15 text-[#9D8CFF] capitalize">{aiTierLabel[profile.ai_tier as string] || profile.ai_tier}</span>}
        </div>
        ...
        <div className="space-y-2">
          {hasCvAccess ? (
            <a href="/cv-ready" className="block text-center bg-gradient-to-r from-[#9D8CFF] to-[#9D8CFF] text-[#060609] text-xs font-black py-2.5 rounded-xl hover:opacity-90 transition-opacity">Open CV Kit →</a>
          ) : (
            <a href="/profile/print" target="_blank" className="block text-center bg-gradient-to-r from-[#9D8CFF] to-[#9D8CFF] text-[#060609] text-xs font-black py-2.5 rounded-xl hover:opacity-90 transition-opacity">Download CV ↓</a>
          )}
          <div className="grid grid-cols-2 gap-2">
            ...
            <a href={`/p/${user.id.slice(0, 8)}`} target="_blank" rel="noopener noreferrer" className="text-center text-[#9D8CFF] text-xs font-bold py-2 rounded-xl border border-[#9D8CFF]/30 hover:border-[#9D8CFF]/60 transition-colors">View public</a>
          </div>
        </div>
      </div>
      ...
      <div className="gradient-border-card rounded-2xl px-4 py-3 flex items-center gap-3">
        ...
        <p className="text-[#A6A6B4] text-xs leading-relaxed">
          {isLive ? <>Your profile is <span className="text-[#9D8CFF] font-semibold">live</span> — companies can find you. 🎉</> : <>Being <span className="text-[#F4F4F7] font-semibold">verified</span> — live once references are confirmed.</>}
        </p>
      </div>
    </aside>
  )
    ...
    <>
      ...
      {(() => {
        ...
        const statusMeta: Record<string, { label: string; color: string }> = {
          actively_looking: { label: 'Actively looking', color: '#9D8CFF' },
          open: { label: 'Open to offers', color: '#9D8CFF' },
        ...
        if (!sm && !primaryBand && pivots.length === 0 && openTo.length === 0 && targetRoles.length === 0 && targetIndustries.length === 0) {
          return (
            <div className="gradient-border-card rounded-2xl p-5 flex items-center justify-between gap-4">
              ...
              <Link href="/profile/edit" className="text-[#9D8CFF] text-sm font-bold whitespace-nowrap">Add →</Link>
            </div>
          )
        }
        return (
          <div className="gradient-border-card rounded-2xl p-6">
            ...
            <div className="ml-4 space-y-4">
              ...
              {openTo.length > 0 && (
                <div className="flex items-center gap-2 flex-wrap"><span className="text-[#7E7E8E] text-[11px] font-bold uppercase tracking-wider">Open to:</span>{openTo.map((e, i) => <span key={i} className="text-[11px] font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(157, 140, 255, 0.12)', color: '#9D8CFF' }}>{engLabel[e] || e}</span>)}</div>
              )}
              {(targetRoles.length > 0 || targetIndustries.length > 0) && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    ...
                    {targetIndustries.map((ind, i) => <span key={`i${i}`} className="text-[11px] font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(157, 140, 255, 0.10)', color: '#9D8CFF' }}>{indLabel[ind] || ind}</span>)}
                  </div>
                </div>
              )}
              {primaryBand && (
              ...
              {pivots.length > 0 && (
                <div>
                  ...
                  <div className="space-y-1.5">{pivots.map((p, i) => <div key={i} className="flex items-baseline gap-2 flex-wrap"><span className="text-[#C7C7D1] text-sm font-bold">{p.track || 'Track'}</span><span className="text-[#9D8CFF] text-sm font-bold">{band(p) || '—'}</span>{p.note && <span className="text-[#7E7E8E] text-xs">· {p.note}</span>}</div>)}</div>
                </div>
              )}
            </div>
          </div>
        ...
        return (
          <div className="gradient-border-card rounded-2xl p-6">
            <div className="flex items-center gap-2.5 mb-3">
              ...
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(157, 140, 255, 0.15)', color: '#9D8CFF' }}>◆ across all references</span>
            </div>
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              {(report.claims_verified?.length ?? 0) > 0 && (
                <div className="rounded-xl p-4" style={{ background: 'rgba(16,185,129,0.12)', border: '1px solid rgba(16,185,129,0.22)' }}>
                  <p className="text-[#9D8CFF] text-[11px] font-bold uppercase tracking-wider mb-2.5">✓ Independently confirmed</p>
                  <ul className="space-y-2">
                    {report.claims_verified!.map((c, i) => (
                      <li key={i} className="flex gap-2 text-[#C7C7D1] text-xs leading-relaxed">
                        <svg className="w-3.5 h-3.5 flex-shrink-0 mt-0.5 text-[#9D8CFF]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>
                        <span>{c}</span>
                      </li>
                    ))}
                  </ul>
              ...
              {(report.top_skills?.length ?? 0) > 0 && (
                <div className="rounded-xl p-4" style={{ background: 'rgba(157, 140, 255, 0.12)', border: '1px solid rgba(157, 140, 255, 0.22)' }}>
                  <p className="text-[#9D8CFF] text-[11px] font-bold uppercase tracking-wider mb-2.5">Most-cited strengths</p>
                  <div className="flex flex-wrap gap-1.5">{report.top_skills!.map((s, i) => <span key={i} className="text-[11px] font-bold px-2.5 py-1 rounded-full" style={{ background: 'rgba(157, 140, 255, 0.14)', color: '#9D8CFF' }}>{s}</span>)}</div>
                </div>
              )}
            </div>
            {(report.conflicts?.length ?? 0) > 0 && (
              <div className="mt-3 rounded-xl p-4" style={{ background: 'rgba(217,119,6,0.12)', border: '1px solid rgba(217,119,6,0.18)' }}>
                <p className="text-[#9D8CFF] text-[11px] font-bold uppercase tracking-wider mb-2.5">⚖ Differing perspectives <span className="text-[#7E7E8E] normal-case font-medium">— flagged, not hidden</span></p>
                <div className="space-y-2.5">
                  {report.conflicts!.map((c, i) => (
                    <div key={i} className="flex gap-2.5">
                      <span className="flex-shrink-0 w-1.5 h-1.5 rounded-full mt-1.5" style={{ background: '#9D8CFF' }} />
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={i} className="flex items-center gap-3">
              <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${item.done ? 'bg-[#9D8CFF]/20' : 'bg-white/[0.05]'}`}>
                {item.done ? <svg className="w-3 h-3 text-[#9D8CFF]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg> : <div className="w-1.5 h-1.5 rounded-full bg-white/[0.05]" />}
              </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <Link href="/profile/references" className="block gradient-border-card rounded-2xl p-5 hover:bg-white/[0.05] transition-colors">
          <div className="flex items-start justify-between gap-2 mb-1">
            ...
            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full flex-shrink-0" style={{ background: 'rgba(157, 140, 255, 0.15)', color: '#9D8CFF' }}>{refScore.jobsComplete} of 2 jobs</span>
          </div>
          ...
          <p className="text-[#9D8CFF] text-xs font-semibold mt-3">{refScore.jobsComplete === 2 ? 'View references →' : 'Add references →'}</p>
        </Link>
        <Link href="/evidence" className="block gradient-border-card rounded-2xl p-5 hover:bg-white/[0.05] transition-colors">
          <div className="flex items-start justify-between gap-2 mb-1">
            ...
            {hasEvidence ? <span className="text-[10px] font-bold bg-emerald-500/15 text-emerald-400 px-2 py-0.5 rounded-full flex-shrink-0">{evidenceCount} file{evidenceCount !== 1 ? 's' : ''} ✓</span> : <span className="text-[10px] font-bold bg-[#9D8CFF]/12 text-[#9D8CFF] px-2 py-0.5 rounded-full flex-shrink-0">Optional</span>}
          </div>
          ...
          <p className="text-[#9D8CFF] text-xs font-semibold mt-3">{hasEvidence ? 'Add more →' : 'Upload evidence →'}</p>
        </Link>
      </div>
        ...
        return (
          <div className="gradient-border-card rounded-2xl p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(157, 140, 255, 0.12)', color: '#9D8CFF' }}>NEW</span>
            </div>
            <div className="space-y-3">
              {entries.map(([lang, s]) => (
            ...
            {proficiency && (proficiency.cefr_level || proficiency.english_level || proficiency.ielts_equivalent) && (
              <div className="pt-3 border-t border-white/[0.08]">
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  {proficiency.cefr_level && <span title={`CEFR ${proficiency.cefr_level} — ${cefrInfo[proficiency.cefr_level] || ''}`} className="text-xs font-bold px-2 py-1 rounded-full cursor-help" style={{ background: 'rgba(157, 140, 255, 0.12)', color: '#9D8CFF' }}>{proficiency.cefr_level} CEFR ⓘ</span>}
                  {proficiency.ielts_equivalent && <span title={`IELTS ${proficiency.ielts_equivalent} band`} className="text-xs font-bold px-2 py-1 rounded-full cursor-help" style={{ background: 'rgba(157, 140, 255, 0.12)', color: '#9D8CFF' }}>IELTS {proficiency.ielts_equivalent} ⓘ</span>}
                  {proficiency.english_level && proficiency.english_level !== 'unassessed' && <span title={`English CEFR ${proficiency.english_level}`} className="text-xs font-bold px-2 py-1 rounded-full cursor-help" style={{ background: 'rgba(157, 140, 255, 0.12)', color: '#9D8CFF' }}>{proficiency.english_level} English ⓘ</span>}
                </div>
          ...
          <div className="space-y-6">
            {workHistory.map((job, i) => (
              <div key={i} className={i > 0 ? 'pt-6 border-t border-white/[0.08]' : ''}>
                <div className="flex items-start justify-between gap-2 mb-1">
                  <div><p className="text-[#F4F4F7] font-bold">{job.title || '—'}</p><p className="text-sm font-semibold" style={{ color: '#9D8CFF' }}>{job.company || '—'}</p></div>
            ...
            <div className="grid sm:grid-cols-2 gap-2">
              {links.map((l, i) => (
                <a key={i} href={l.url!.startsWith('http') ? l.url! : `https://${l.url}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2.5 bg-white/[0.05] hover:bg-white/[0.07] rounded-lg px-3 py-2 transition-colors">
                  <span className="text-[#9D8CFF] text-xs font-bold w-8 flex-shrink-0">{l.icon}</span>
    ...
    <div className="space-y-4">
      <a href="/translate" className="block gradient-border-card rounded-2xl p-5 hover:bg-white/[0.03] transition-colors">
        <div className="flex items-center justify-between gap-3">
          ...
          <span className="text-[#9D8CFF] text-sm font-bold flex-shrink-0">Open →</span>
        </div>
      </a>
    ...
    <>
      ...
      {profile.skill_quadrant && (
        <div className="gradient-border-card rounded-2xl p-6">
          <div className="flex items-center gap-2.5 mb-1">
            <span className="w-1.5 h-6 rounded-full" style={{ background: 'linear-gradient(180deg,#9D8CFF, #34D399)' }} />
            <h2 className="text-[#F4F4F7] font-black text-xl tracking-tight">Skill Fingerprint</h2>
            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(157, 140, 255, 0.14)', color: '#9D8CFF' }}>◆ Shapi-assessed</span>
          </div>
          ...
          return (
            <div className="gradient-border-card rounded-2xl p-6">
              <div className="flex items-center justify-between gap-4">
                ...
                <a href="/work-style" className="flex-shrink-0 px-4 py-2.5 rounded-xl font-black text-xs" style={{ background: 'linear-gradient(135deg,#9D8CFF, #34D399)', color: '#060609' }}>Take it →</a>
              </div>
            </div>
          )
        }
        return (
          <div className="gradient-border-card rounded-2xl p-6">
            <div className="flex items-center gap-2.5 mb-4">
              ...
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(157, 140, 255, 0.12)', color: '#9D8CFF' }}>✎ Self-assessment</span>
              ...
              {dims.map(d => {
                ...
                return (
                  <div key={d.key}>
                    ...
                    <div className="relative h-2 rounded-full" style={{ background: 'rgba(255,255,255,0.07)' }}><div className="absolute top-1/2 -translate-y-1/2 w-3 h-3 rounded-full" style={{ left: `calc(${score}% - 6px)`, background: 'linear-gradient(135deg,#9D8CFF, #34D399)' }} /></div>
                  </div>
                )
              })}
            </div>
      ...
      <div className="gradient-border-card rounded-2xl p-6">
        <div className="flex items-center gap-2.5 mb-1">
          ...
          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(157, 140, 255, 0.15)', color: '#9D8CFF' }}>✓ Verified · coming soon</span>
        </div>
      ...
      <style>{`
        .gradient-border-card {
          background: linear-gradient(#0D0C14, #0D0C14) padding-box,
                      linear-gradient(135deg, rgba(157, 140, 255, 0.18), rgba(157, 140, 255, 0.12)) border-box;
