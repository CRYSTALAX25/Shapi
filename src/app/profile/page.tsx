...
export default async function ProfilePage() {
    ...
    <aside className="lg:sticky lg:top-6 space-y-4 self-start">
      <div className="gradient-border-card rounded-2xl p-6">
        <div className="flex items-center gap-4 mb-4">
          {profile.profile_image_url ? (
            ...
            <div className="w-16 h-16 rounded-full flex-shrink-0 flex items-center justify-center text-2xl font-black text-[#060609]" style={{ background: 'linear-gradient(135deg,#22D3EE,#A78BFA)' }}>
              {((profile.full_name as string) || '?').charAt(0).toUpperCase()}
            </div>
          )}
          <div className="min-w-0">
            <h1 className="text-xl font-black text-[#0E0E1A] leading-tight">{profile.full_name || 'Your Name'}</h1>
            <p className="text-[#5A5A6E] text-sm">{profile.headline || 'Professional'}</p>
          </div>
        </div>

        {profile.location && <p className="text-[#8A8A99] text-sm mb-3">📍 {profile.location}</p>}

        {/* Completion + live */}
        <div className="flex items-center gap-3 mb-3">
          <div className="text-3xl font-black" style={{ background: 'linear-gradient(135deg,#22D3EE,#A78BFA)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>{completion}%</div>
          <div className="flex-1">
            <div className="h-1.5 rounded-full overflow-hidden" style={{ background: 'rgba(14,14,26,0.06)' }}>
              <div className="h-full rounded-full" style={{ width: `${completion}%`, background: 'linear-gradient(90deg,#22D3EE,#A78BFA)' }} />
            </div>
            <p className="text-[#8A8A99] text-[10px] mt-1">profile strength</p>
          </div>
        </div>

        {/* Badges */}
        <div className="flex flex-wrap gap-1.5 mb-4">
          {isLive && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-[#22D3EE]/15 text-[#0891B2]">✓ Live</span>}
          ...
          {profile.ai_tier && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-[#A78BFA]/15 text-[#7C3AED] capitalize">{aiTierLabel[profile.ai_tier as string] || profile.ai_tier}</span>}
        </div>
        ...
        {(() => {
          ...
          return (
            <div className="mb-4">
              <p className="text-[#8A8A99] text-[10px] font-bold uppercase tracking-wider mb-1.5">Right to work</p>
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <Link href="/profile/edit" className="text-center text-[#5A5A6E] text-xs font-bold py-2 rounded-xl border border-[#0E0E1A]/[0.08] hover:border-[#0E0E1A]/20 transition-colors">Edit</Link>
            <a href={`/p/${user.id.slice(0, 8)}`} target="_blank" rel="noopener noreferrer" className="text-center text-[#0891B2] text-xs font-bold py-2 rounded-xl border border-[#22D3EE]/30 hover:border-[#22D3EE]/60 transition-colors">View public</a>
          </div>
        </div>
      </div>
      ...
      <div className="gradient-border-card rounded-2xl px-4 py-3 flex items-center gap-3">
        ...
        <p className="text-[#5A5A6E] text-xs leading-relaxed">
          {isLive ? <>Your profile is <span className="text-[#0891B2] font-semibold">live</span> — companies can find you. 🎉</> : <>Being <span className="text-[#0E0E1A] font-semibold">verified</span> — live once references are confirmed.</>}
        </p>
      </div>
    </aside>
  )
  ...
  const overviewPanel = (
    <>
        ...
        return (
          <div className="gradient-border-card rounded-2xl p-6">
            <div className="flex items-center gap-2.5 mb-4">
              ...
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(14,14,26,0.06)', color: '#8A8A99' }}>🔒 private</span>
            </div>
            <div className="ml-4 space-y-4">
              ...
              {openTo.length > 0 && (
                <div className="flex items-center gap-2 flex-wrap"><span className="text-[#8A8A99] text-[11px] font-bold uppercase tracking-wider">Open to:</span>{openTo.map((e, i) => <span key={i} className="text-[11px] font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(167,139,250,0.12)', color: '#7C3AED' }}>{engLabel[e] || e}</span>)}</div>
              )}
              {(targetRoles.length > 0 || targetIndustries.length > 0) && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    ...
                    {targetIndustries.map((ind, i) => <span key={`i${i}`} className="text-[11px] font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(124,58,237,0.10)', color: '#7C3AED' }}>{indLabel[ind] || ind}</span>)}
                  </div>
                </div>
              )}
              {primaryBand && (
                <div>
                  ...
                  <p className="text-[#0E0E1A] text-lg font-black">{primaryBand}{se?.flexible && <span className="text-[#8A8A99] text-xs font-medium ml-2">negotiable</span>}{se?.includes_allowances && <span className="text-[#8A8A99] text-xs font-medium ml-2">incl. allowances</span>}</p>
                </div>
              )}
              {pivots.length > 0 && (
                <div>
                  <p className="text-[#8A8A99] text-[11px] font-bold uppercase tracking-wider mb-2">Open to pivot tracks</p>
                  <div className="space-y-1.5">{pivots.map((p, i) => <div key={i} className="flex items-baseline gap-2 flex-wrap"><span className="text-[#3F3F4E] text-sm font-bold">{p.track || 'Track'}</span><span className="text-[#0891B2] text-sm font-bold">{band(p) || '—'}</span>{p.note && <span className="text-[#8A8A99] text-xs">· {p.note}</span>}</div>)}</div>
                </div>
              )}
            </div>
          </div>
      ...
      {profile.summary && (
        <div className="gradient-border-card rounded-2xl p-6">
          <h2 className="text-[#0E0E1A] font-black text-sm uppercase tracking-widest mb-3 opacity-50">About</h2>
    ...
    <>
      {tm && (
        <div className="gradient-border-card rounded-2xl px-5 py-4 flex items-center gap-4" style={{ background: tm.bg, border: `1px solid ${tm.color}33` }}>
          ...
          <div><p className="font-bold text-sm" style={{ color: tm.color }}>{tm.label}</p><p className="text-[#5A5A6E] text-xs mt-0.5">{tm.description}</p></div>
        </div>
      )}
          ...
          {[
            ...
            <div key={i} className="flex items-center gap-3">
              <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${item.done ? 'bg-[#22D3EE]/20' : 'bg-[#0E0E1A]/[0.04]'}`}>
                {item.done ? <svg className="w-3 h-3 text-[#0891B2]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg> : <div className="w-1.5 h-1.5 rounded-full bg-[#0E0E1A]/[0.04]" />}
              </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <Link href="/profile/references" className="block gradient-border-card rounded-2xl p-5 hover:bg-[#0E0E1A]/[0.03] transition-colors">
          <div className="flex items-start justify-between gap-2 mb-1">
            ...
            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full flex-shrink-0" style={{ background: refScore.jobsComplete === 2 ? 'rgba(52,211,153,0.15)' : refScore.jobsComplete === 1 ? 'rgba(251,191,36,0.15)' : 'rgba(251,113,133,0.15)', color: refScore.jobsComplete === 2 ? '#34D399' : refScore.jobsComplete === 1 ? '#FBBF24' : '#FB7185' }}>{refScore.jobsComplete} of 2 jobs</span>
          </div>
          <p className="text-[#8A8A99] text-xs leading-relaxed">{refScore.jobsComplete === 2 ? 'Both jobs fully verified — profile live ✓' : refScore.jobsComplete === 1 ? 'One more job to verify (85% → 100%).' : 'Add your 2 latest managers — we contact them, they nominate a colleague + stakeholder.'}</p>
          <p className="text-[#0891B2] text-xs font-semibold mt-3">{refScore.jobsComplete === 2 ? 'View references →' : 'Add references →'}</p>
        </Link>
        <Link href="/evidence" className="block gradient-border-card rounded-2xl p-5 hover:bg-[#0E0E1A]/[0.03] transition-colors">
          <div className="flex items-start justify-between gap-2 mb-1">
            ...
            {hasEvidence ? <span className="text-[10px] font-bold bg-emerald-500/15 text-emerald-600 px-2 py-0.5 rounded-full flex-shrink-0">{evidenceCount} file{evidenceCount !== 1 ? 's' : ''} ✓</span> : <span className="text-[10px] font-bold bg-[#FB7185]/12 text-[#E11D48] px-2 py-0.5 rounded-full flex-shrink-0">Optional</span>}
          </div>
          <p className="text-[#8A8A99] text-xs">{hasEvidence ? 'Photos and docs uploaded — strengthens your profile.' : 'Photos, docs, references that prove your track record'}</p>
          <p className="text-[#0891B2] text-xs font-semibold mt-3">{hasEvidence ? 'Add more →' : 'Upload evidence →'}</p>
        </Link>
      </div>
            ...
            <div className="space-y-3">
              {entries.map(([lang, s]) => (
                <div key={lang} className="bg-[#0E0E1A]/[0.04] rounded-xl p-3 border border-[#0E0E1A]/[0.08]">
                  <div className="flex items-center justify-between mb-2"><span className="text-[#0E0E1A] text-sm font-bold capitalize">{s.language || lang}</span>{s.duration_s && <span className="text-[#8A8A99] text-[10px]">{s.duration_s}s</span>}</div>
                  ...
                  {s.transcript && <p className="text-[#5A5A6E] text-[11px] mt-2 italic line-clamp-2">&ldquo;{s.transcript}&rdquo;</p>}
                </div>
              ))}
            </div>
          </div>
      ...
      {(() => {
        ...
        return (
          <div className="gradient-border-card rounded-2xl p-6">
            ...
            {langs.length > 0 && <div className="space-y-2 mb-4">{langs.map((l, i) => <div key={i} className="flex items-center justify-between gap-2"><span className="text-[#0E0E1A] text-sm">{l.language}</span><span className="text-[#8A8A99] text-xs">{l.level}</span></div>)}</div>}
            {proficiency && (proficiency.cefr_level || proficiency.english_level || proficiency.ielts_equivalent) && (
              <div className="pt-3 border-t border-[#0E0E1A]/[0.08]">
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  {proficiency.cefr_level && <span title={`CEFR ${proficiency.cefr_level} — ${cefrInfo[proficiency.cefr_level] || ''}`} className="text-xs font-bold px-2 py-1 rounded-full cursor-help" style={{ background: 'rgba(34,211,238,0.08)', color: '#0891B2' }}>{proficiency.cefr_level} CEFR ⓘ</span>}
                  {proficiency.ielts_equivalent && <span title={`IELTS ${proficiency.ielts_equivalent} band`} className="text-xs font-bold px-2 py-1 rounded-full cursor-help" style={{ background: 'rgba(167,139,250,0.08)', color: '#7C3AED' }}>IELTS {proficiency.ielts_equivalent} ⓘ</span>}
                  {proficiency.english_level && proficiency.english_level !== 'unassessed' && <span title={`English CEFR ${proficiency.english_level}`} className="text-xs font-bold px-2 py-1 rounded-full cursor-help" style={{ background: 'rgba(251,113,133,0.08)', color: '#E11D48' }}>{proficiency.english_level} English ⓘ</span>}
                </div>
                <p className="text-[#8A8A99] text-[10px] leading-relaxed">A1/A2 beginner · B1/B2 intermediate (B2 = fluent professional) · C1/C2 advanced</p>
              </div>
            )}
          </div>
        )
  ...
  const experiencePanel = (
    <>
      {workHistory.length > 0 ? (
        ...
        <div className="gradient-border-card rounded-2xl p-8 text-center"><p className="text-[#8A8A99] text-sm">Work history will appear here once your CV is processed</p></div>
      )}

      {skills.length > 0 && (
        <div className="gradient-border-card rounded-2xl p-6">
          <h2 className="text-[#0E0E1A] font-black text-sm uppercase tracking-widest mb-4 opacity-50">Skills</h2>
          <div className="flex flex-wrap gap-2">{skills.map((skill, i) => <span key={i} className="bg-[#0E0E1A]/[0.04] text-[#3F3F4E] text-xs px-3 py-1.5 rounded-full">{skill}</span>)}</div>
        </div>
      )}
      ...
      {(() => {
        ...
        return (
          <div className="gradient-border-card rounded-2xl p-5">
            <div className="flex items-start justify-between gap-2 mb-3"><p className="text-[#0E0E1A] font-bold text-sm">Online presence</p><Link href="/profile/edit" className="text-[#8A8A99] text-[10px] hover:text-[#3F3F4E] flex-shrink-0">Edit</Link></div>
            <div className="grid sm:grid-cols-2 gap-2">
              {links.map((l, i) => (
                <a key={i} href={l.url!.startsWith('http') ? l.url! : `https://${l.url}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2.5 bg-[#0E0E1A]/[0.04] hover:bg-[#0E0E1A]/[0.06] rounded-lg px-3 py-2 transition-colors">
                  <span className="text-[#0891B2] text-xs font-bold w-8 flex-shrink-0">{l.icon}</span>
                  <span className="min-w-0"><span className="text-[#0E0E1A] text-xs font-bold block">{l.label}</span><span className="text-[#8A8A99] text-[11px] truncate block">{tidy(l.url!)}</span></span>
          ...
          return (
            <div className="gradient-border-card rounded-2xl p-6">
              <div className="flex items-center justify-between gap-4">
                <div><h2 className="text-[#0E0E1A] font-black text-base mb-1">Work-style check <span className="text-[10px] font-bold px-2 py-0.5 rounded-full align-middle" style={{ background: 'rgba(14,14,26,0.04)', color: '#5A5A6E' }}>optional · 2 min</span></h2><p className="text-[#5A5A6E] text-xs">Show companies how you prefer to work — team vs solo, leader vs contributor, and more.</p></div>
          ...
          <div className="gradient-border-card rounded-2xl p-6">
            ...
            <div className="grid sm:grid-cols-2 gap-x-8 gap-y-3">
              {dims.map(d => {
                ...
                return (
                  <div key={d.key}>
                    <div className="flex items-center justify-between text-[11px] mb-1"><span className="text-[#5A5A6E]">{d.poleB}</span><span className="text-[#0E0E1A] font-bold">{label}</span><span className="text-[#5A5A6E]">{d.poleA}</span></div>
                    <div className="relative h-2 rounded-full" style={{ background: 'rgba(14,14,26,0.06)' }}><div className="absolute top-1/2 -translate-y-1/2 w-3 h-3 rounded-full" style={{ left: `calc(${score}% - 6px)`, background: 'linear-gradient(135deg,#22D3EE,#A78BFA)' }} /></div>
                  </div>
                )
              })}
            </div>
  ...
  return (
    <div className="min-h-screen bg-[#F1F2F7]">
      ...
      <nav className="relative z-10 px-6 py-4 flex items-center justify-between max-w-6xl mx-auto border-b border-[#0E0E1A]/[0.08]">
