    ...
    <aside className="lg:sticky lg:top-6 space-y-4 self-start">
      <div className="gradient-border-card rounded-2xl p-6">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="min-w-0">
            <h1 className="text-xl font-black leading-tight" style={{ color: '#F08CAE' }}>{profile.full_name || 'Your Name'}</h1>
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <a href={`/p/${user.id.slice(0, 8)}`} target="_blank" rel="noopener noreferrer" className="text-center text-[#6AA8F5] text-xs font-bold py-2 rounded-xl border border-[#6AA8F5]/30 hover:border-[#6AA8F5]/60 transition-colors">View public</a>
          </div>
        </div>
      </div>
      ...
      <div className="gradient-border-card rounded-2xl px-4 py-3 flex items-center gap-3">
        ...
        <p className="text-[#A6A6B4] text-xs leading-relaxed">
          {isLive ? <>Your profile is <span className="text-[#6AA8F5] font-semibold">live</span> — companies can find you. 🎉</> : <>Being <span className="text-[#F4F4F7] font-semibold">verified</span> — live once references are confirmed.</>}
        </p>
      </div>
    </aside>
  )
    ...
    <>
      ...
      {(() => {
        ...
        const statusMeta: Record<string, { label: string; color: string }> = {
          actively_looking: { label: 'Actively looking', color: '#6AA8F5' },
          open: { label: 'Open to offers', color: '#6AA8F5' },
        ...
        if (!sm && !primaryBand && pivots.length === 0 && openTo.length === 0 && targetRoles.length === 0 && targetIndustries.length === 0) {
          return (
            <div className="gradient-border-card rounded-2xl p-5 flex items-center justify-between gap-4">
              ...
              <Link href="/profile/edit" className="text-[#6AA8F5] text-sm font-bold whitespace-nowrap">Add →</Link>
            </div>
          )
        }
        return (
            ...
            <div className="ml-4 space-y-4">
              ...
              {pivots.length > 0 && (
                <div>
                  ...
                  <div className="space-y-1.5">{pivots.map((p, i) => <div key={i} className="flex items-baseline gap-2 flex-wrap"><span className="text-[#C7C7D1] text-sm font-bold">{p.track || 'Track'}</span><span className="text-[#6AA8F5] text-sm font-bold">{band(p) || '—'}</span>{p.note && <span className="text-[#7E7E8E] text-xs">· {p.note}</span>}</div>)}</div>
                </div>
              )}
            </div>
          </div>
        ...
        return (
          <div className="gradient-border-card rounded-2xl p-6">
            <div className="flex items-center gap-2.5 mb-3">
              ...
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(106,168,245,0.15)', color: '#6AA8F5' }}>◆ across all references</span>
            </div>
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              {(report.claims_verified?.length ?? 0) > 0 && (
                <div className="rounded-xl p-4" style={{ background: 'rgba(16,185,129,0.12)', border: '1px solid rgba(16,185,129,0.22)' }}>
                  <p className="text-[#6AA8F5] text-[11px] font-bold uppercase tracking-wider mb-2.5">✓ Independently confirmed</p>
                  <ul className="space-y-2">
                    {report.claims_verified!.map((c, i) => (
                      <li key={i} className="flex gap-2 text-[#C7C7D1] text-xs leading-relaxed">
                        <svg className="w-3.5 h-3.5 flex-shrink-0 mt-0.5 text-[#6AA8F5]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>
                        <span>{c}</span>
                      </li>
                    ))}
                  </ul>
              ...
              {(report.top_skills?.length ?? 0) > 0 && (
                <div className="rounded-xl p-4" style={{ background: 'rgba(106,168,245,0.12)', border: '1px solid rgba(106,168,245,0.22)' }}>
                  <p className="text-[#6AA8F5] text-[11px] font-bold uppercase tracking-wider mb-2.5">Most-cited strengths</p>
                  <div className="flex flex-wrap gap-1.5">{report.top_skills!.map((s, i) => <span key={i} className="text-[11px] font-bold px-2.5 py-1 rounded-full" style={{ background: 'rgba(106,168,245,0.14)', color: '#6AA8F5' }}>{s}</span>)}</div>
                </div>
              )}
            </div>
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={i} className="flex items-center gap-3">
              <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${item.done ? 'bg-[#6AA8F5]/20' : 'bg-white/[0.05]'}`}>
                {item.done ? <svg className="w-3 h-3 text-[#6AA8F5]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg> : <div className="w-1.5 h-1.5 rounded-full bg-white/[0.05]" />}
              </div>
        ...
        return (
          <div className="gradient-border-card rounded-2xl p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(106,168,245,0.12)', color: '#6AA8F5' }}>NEW</span>
            </div>
            <div className="space-y-3">
              {entries.map(([lang, s]) => (
          ...
          <div className="space-y-6">
            {workHistory.map((job, i) => (
              <div key={i} className={i > 0 ? 'pt-6 border-t border-white/[0.08]' : ''}>
                <div className="flex items-start justify-between gap-2 mb-1">
                  <div><p className="text-[#F4F4F7] font-bold">{job.title || '—'}</p><p className="text-sm font-semibold" style={{ color: '#6AA8F5' }}>{job.company || '—'}</p></div>
            ...
            <div className="grid sm:grid-cols-2 gap-2">
              {links.map((l, i) => (
                <a key={i} href={l.url!.startsWith('http') ? l.url! : `https://${l.url}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2.5 bg-white/[0.05] hover:bg-white/[0.07] rounded-lg px-3 py-2 transition-colors">
                  <span className="text-[#6AA8F5] text-xs font-bold w-8 flex-shrink-0">{l.icon}</span>
