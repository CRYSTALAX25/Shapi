      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 pt-8 pb-20">
        ...
        <div className="rounded-2xl p-5 mb-6" style={cardStyle}>
          ...
          <button onClick={suggestFigures} disabled={suggestLoading || !field.trim()}
            className="mb-4 text-xs font-bold px-3 py-1.5 rounded-full disabled:opacity-40" style={{ background: 'rgba(56, 189, 248, 0.12)', color: '#38BDF8', border: '1px solid rgba(56, 189, 248, 0.25)' }}>
            {suggestLoading ? 'Thinking…' : `💡 Suggest typical figures${field.trim() ? ` for ${field.trim()}` : ''}`}
          </button>
          {suggested && (
            <p className="text-[#38BDF8] text-xs mb-4 -mt-2">Pre-filled with typical figures for {field || 'this field'}{country ? ` in ${country}` : ''}{bp?.pricing_suggestion?.note ? ` — ${bp.pricing_suggestion.note}` : ''}. Adjust to your real costs.</p>
          )}
          ...
          <div className="grid grid-cols-3 gap-2 mt-4">
            ...
            <div className="rounded-xl p-3" style={{ background: 'rgba(255,255,255,0.05)' }}>
              ...
              <p className="text-sm font-black mt-0.5" style={{ color: '#38BDF8' }}>{currency} {fmt(profit)}</p>
            </div>
            <div className="rounded-xl p-3" style={{ background: 'rgba(56, 189, 248, 0.12)', border: '1px solid rgba(56, 189, 248, 0.3)' }}>
              <p className={labelCls}>Quote client</p>
              <p className="text-sm font-black mt-0.5" style={{ color: '#38BDF8' }}>{currency} {fmt(quote)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl p-5" style={cardStyle}>
          ...
          {bp && (() => {
            ...
            const fitColor = (bp.fit?.score ?? 0) >= 7 ? '#38BDF8' : (bp.fit?.score ?? 0) >= 4 ? '#38BDF8' : '#FB7185'
            return (
            <div className="mt-5 space-y-4">
              ...
              {bp.verdict && (() => {
                ...
                const vc = go ? '#38BDF8' : rec.includes('caution') ? '#FBBF24' : '#FB7185'
                ...
                <div className="rounded-xl p-4" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)' }}>
                  ...
                  <div className="grid sm:grid-cols-2 gap-3">
                    {bp.fit.strengths && bp.fit.strengths.length > 0 && (
                      <div><p className="text-[#38BDF8] text-[10px] font-bold uppercase mb-1">✓ You already bring</p>
              ...
              {bp.ownership && (() => {
                ...
                const oc = can === 'yes' ? '#38BDF8' : can === 'no' ? '#FB7185' : '#FBBF24'
              ...
              {bp.automation && (bp.automation.tasks?.length ?? 0) > 0 && (
                <div className="rounded-xl p-4" style={{ background: 'rgba(56, 189, 248, 0.08)', border: '1px solid rgba(56, 189, 248, 0.2)' }}>
                  <p className="text-[#38BDF8] text-[10px] font-bold uppercase tracking-wider mb-2">🤖 What you can automate (cut labour cost)</p>
                  <ul className="space-y-1 mb-2">{bp.automation.tasks!.map((t, i) => <li key={i} className="text-[#C7C7D1] text-xs flex gap-2"><span className="text-[#38BDF8]">•</span><span>{t}</span></li>)}</ul>
              ...
              {((bp.pros?.length ?? 0) > 0 || (bp.cons?.length ?? 0) > 0) && (
                <div className="grid sm:grid-cols-2 gap-3">
                  {bp.pros && bp.pros.length > 0 && (
                    <div className="rounded-xl p-4" style={{ background: 'rgba(56, 189, 248, 0.08)', border: '1px solid rgba(56, 189, 248, 0.2)' }}>
                      <p className="text-[#38BDF8] text-[10px] font-bold uppercase tracking-wider mb-2">👍 Pros in {bp.country}</p>
              ...
              {((bp.structures?.length ?? 0) > 0 || bp.time_estimate) && (
                <div className="flex flex-wrap items-center gap-2 text-xs">
                  ...
                  {bp.time_estimate && <span className="font-bold px-3 py-1.5 rounded-full" style={{ background: 'rgba(56, 189, 248, 0.12)', color: '#38BDF8' }}>⏱️ {bp.time_estimate}</span>}
                </div>
              )}
                  ...
                  <div className="space-y-2">
                    {bp.launch_plan.map((ph, i) => (
                      <div key={i} className="rounded-xl p-3" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)' }}>
                        <p className="text-[#F4F4F7] font-bold text-sm mb-1.5"><span className="text-[#38BDF8]">{i + 1}.</span> {ph.phase}</p>
                        <ul className="space-y-1">{(ph.steps || []).map((st, j) => <li key={j} className="text-[#A6A6B4] text-xs flex gap-2"><span className="text-[#38BDF8]">•</span><span>{st}</span></li>)}</ul>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="space-y-2">
                    {bp.entities.map((e, i) => (
                      <a key={i} href={entityUrl(e)} target="_blank" rel="noopener noreferrer"
                        className="flex items-center justify-between gap-3 rounded-xl px-3 py-2.5 hover:opacity-90 transition-opacity" style={{ background: 'rgba(56, 189, 248, 0.10)', border: '1px solid rgba(56, 189, 248, 0.2)' }}>
              ...
              {bp.contacts && bp.contacts.length > 0 && (
                <div>
                  ...
                  <ul className="space-y-1">{bp.contacts.map((s, i) => <li key={i} className="text-[#C7C7D1] text-xs flex gap-2"><span style={{ color: '#38BDF8' }}>•</span><span>{s}</span></li>)}</ul>
                </div>
              )}
              ...
              <button onClick={() => getSteps(true)} disabled={loading}
                className="text-xs font-bold px-3 py-1.5 rounded-full disabled:opacity-40" style={{ background: 'rgba(56, 189, 248, 0.12)', color: '#38BDF8', border: '1px solid rgba(56, 189, 248, 0.25)' }}>
