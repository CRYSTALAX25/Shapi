...
export default function RoleEditForm({ role, fromWhatsApp }: { role: EditableRole; fromWhatsApp: boolean }) {
  ...
  const statusBadge =
    status === 'active' ? { label: 'Active', bg: 'rgba(56, 189, 248, 0.12)', c: '#38BDF8' } :
  ...
  return (
    <>
      ...
      {fromWhatsApp && (
        <div className="rounded-xl px-4 py-3 mb-6" style={{ background: 'rgba(56, 189, 248, 0.08)', border: '1px solid rgba(56, 189, 248, 0.22)' }}>
          <p className="text-sm font-bold text-[#38BDF8]">💬 Drafted from your WhatsApp chat</p>
        ...
        <div className="flex flex-wrap gap-3">
          {status !== 'active' && (
            <button onClick={publish} disabled={saving}
              className="bg-gradient-to-r from-[#38BDF8] to-[#38BDF8] px-6 py-3 rounded-full font-black text-sm text-[#fff] hover:opacity-90 transition-opacity disabled:opacity-50">
