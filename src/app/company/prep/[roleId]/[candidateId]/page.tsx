  ...
  return (
    <div className="min-h-screen bg-[#060609]">
      <nav className="px-6 py-4 border-b border-[rgba(255,255,255,0.08)] max-w-3xl mx-auto flex items-center justify-between">
        <Link href="/company/pipeline" className="font-black text-xl tracking-tighter" style={{ background: 'linear-gradient(135deg,#38BDF8, #34D399)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>shapi</Link>
      ...
      <div className="max-w-3xl mx-auto px-6 pt-8 pb-20">
        <p className="text-xs font-black uppercase tracking-widest mb-2" style={{ color: '#38BDF8' }}>Interview prep · for the hiring manager</p>
        ...
        <div className="flex items-center gap-2 mb-6">
          ...
          {salaryBand && <span className="text-xs font-bold" style={{ color: '#38BDF8' }}>· Expects {salaryBand}</span>}
        </div>

        {report.summary_en && (
          <div className="rounded-2xl p-5 mb-4" style={{ ...card, borderLeft: '3px solid #38BDF8' }}>
        ...
        <div className="grid sm:grid-cols-2 gap-4 mb-4">
          {(report.claims_verified?.length ?? 0) > 0 && (
            <div className="rounded-2xl p-5" style={card}>
              <p className="text-[#38BDF8] text-[11px] font-bold uppercase tracking-wider mb-2">✓ Verified strengths</p>
          ...
          {refSkills.length > 0 && (
            <div className="rounded-2xl p-5" style={card}>
              <p className="text-[#38BDF8] text-[11px] font-bold uppercase tracking-wider mb-2">Most-cited by references</p>
              <div className="flex flex-wrap gap-1.5">{refSkills.map((s, i) => <span key={i} className="text-[11px] font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(56, 189, 248, 0.10)', color: '#38BDF8' }}>{s}</span>)}</div>
            </div>
          )}
        </div>
            ...
            <ol className="space-y-3">
              {questions.map((q, i) => (
                <li key={i} className="flex gap-3">
                  <span className="flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs font-black text-white" style={{ background: 'linear-gradient(135deg,#38BDF8, #34D399)' }}>{i + 1}</span>
