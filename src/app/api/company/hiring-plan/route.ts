...
// the next 3 hires (role, level, mode), an approximate monthly comp burden, and
// risks + quick wins.
//
// HONESTY: an LLM has no live comp data — comp ranges are INDICATIVE, flagged
// as approximate. Auth optional. Fast path via Haiku — no web search.
...
export async function POST(request: Request) {
  ...
  const budgetLine = monthly_burn_budget != null
    ...
    : 'Monthly burn budget for hiring: not provided — assume a typical budget for this stage.'
  const headcountLine = headcount_today != null
    ? `Current headcount: ${headcount_today}.`
    : 'Current headcount: not provided — assume a typical team for this stage.'
...
- Monthly comp estimate is an APPROXIMATE range for the 3 hires combined, in local currency or USD if uncertain. Always include "approx; varies" or similar in the note.
...
Return ONLY valid JSON (tight; each string ≤ 22 words):
{
  ...
  "monthly_comp_estimate": { "currency": "USD or local code/symbol", "low": 0, "high": 0, "note": "approx; varies by seniority and location" },
