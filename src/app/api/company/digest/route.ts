...
export async function POST(request: Request) {
  ...
  for (const company of companies) {
    ...
    const matchColor = (s: number) =>
      s >= 75 ? '#34D399' : s >= 50 ? '#22D3EE' : '#A78BFA'

    const candidateRows = scored.map(c => `
      <div style="background:#0d0d14;border:1px solid rgba(34,211,238,0.1);border-radius:12px;padding:16px;margin-bottom:10px">
        ...
        <a href="${SITE}/company/dashboard" style="font-size:12px;color:#22D3EE;font-weight:700;text-decoration:none">View profile →</a>
      </div>
    `).join('')
