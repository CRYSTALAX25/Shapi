  ...
  try {
    ...
    const calendlyBlockHtml = calendlyUrl
      ? `<div style="margin-top:16px;padding-top:16px;border-top:1px solid rgba(255,255,255,0.06);text-align:center">
           ...
           <a href="${escape(calendlyUrl)}" style="display:inline-block;padding:12px 24px;background:linear-gradient(135deg,#6AA8F5,#F08CAE,#F58E9A);color:#fff;font-size:13px;font-weight:900;border-radius:100px;text-decoration:none">Book a time →</a>
         </div>`
      : ''
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family:-apple-system,sans-serif;max-width:520px;margin:0 auto;padding:32px 24px;background:#0E0E13;border-radius:16px;color:#F4F4F7">
          <p style="font-size:22px;font-weight:900;letter-spacing:-0.5px;margin:0 0 22px;background:linear-gradient(135deg,#6AA8F5,#F08CAE,#F58E9A);-webkit-background-clip:text;-webkit-text-fill-color:transparent">shapi</p>
          <div style="background:#16161F;border:1px solid rgba(255,255,255,0.08);border-radius:14px;padding:24px">
