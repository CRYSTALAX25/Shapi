      ...
      <nav className="px-6 py-5 max-w-5xl mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center gap-6 text-sm">
          ...
          <Link href="/signup" className="font-bold text-white px-4 py-2 rounded-full"
            style={{ background: 'linear-gradient(135deg, #6AA8F5, #F08CAE)' }}>Get started →</Link>
        </div>
      </nav>

      <div className="max-w-5xl mx-auto px-6 pt-10 pb-24">
        <div className="mb-12">
          <p className="text-xs font-black uppercase tracking-widest mb-3" style={{ color: '#F08CAE' }}>The Shapi blog</p>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {BLOG_POSTS.map(post => (
            <Link key={post.slug} href={`/blog/${post.slug}`}
              className="group rounded-2xl p-7 bg-[#16161F] transition-all hover:-translate-y-1"
