    ...
    <div className="min-h-screen bg-[#F8F4EE]">
      ...
      <div className="max-w-4xl mx-auto px-6">
        ...
        <div className="flex justify-between mt-2">
          {steps.map((s, i) => (
            <span key={i} className={`text-xs font-medium ${i <= step ? 'text-[#0B5563]' : 'text-[#1C1C2E]/30'}`}>
              {s}
            </span>
          ))}
        </div>
      ...
      <div className="max-w-2xl mx-auto px-6 pt-12 pb-20">
        ...
        {step === 0 && (
          <div className="space-y-6">
            <div>
              ...
              <p className="text-[#1C1C2E]/60">No CV template. Just tell us who you are.</p>
            </div>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-[#1C1C2E] mb-2">Full name</label>
              ...
              <div>
                <label className="block text-sm font-medium text-[#1C1C2E] mb-2">In your own words — who are you and what are you looking for?</label>
                ...
                <p className="text-xs text-[#1C1C2E]/40 mt-2">Write like you&apos;re explaining to someone at a networking event.</p>
              </div>
            </div>
          </div>
        )}
        ...
        {step === 1 && (
          <div className="space-y-6">
            <div>
              ...
              <p className="text-[#1C1C2E]/60">Add your most relevant roles. We verify these independently.</p>
            </div>
            {work.map((w, i) => (
              <div key={i} className="bg-white rounded-2xl p-6 border border-[#1C1C2E]/10 space-y-4">
                <p className="text-sm font-medium text-[#0B5563]">{i === 0 ? 'Most recent role' : `Role ${i + 1}`}</p>
            ...
            <button
              ...
              className="text-sm text-[#0B5563] font-medium flex items-center gap-2"
            >
        ...
        {step === 2 && (
          <div className="space-y-6">
            <div>
              ...
              <p className="text-[#1C1C2E]/60">We verify by evidence — not just what you claim.</p>
            </div>
            <div className="bg-white rounded-2xl p-6 border border-[#1C1C2E]/10 space-y-4">
              <p className="text-sm font-medium text-[#1C1C2E]">How do you use AI in your work?</p>
              {[
                ...
                <label key={opt.val} onClick={() => setAiTier(opt.val as 'user' | 'integrator' | 'builder')}
                  ...
                  <div className={`w-5 h-5 rounded-full border-2 mt-0.5 flex-shrink-0 transition-colors ${aiTier === opt.val ? 'border-[#0B5563] bg-[#0B5563]' : 'border-[#0B5563]/30 group-hover:border-[#0B5563]'}`} />
                  <div>
                    <p className="text-sm text-[#1C1C2E] font-medium">{opt.label}</p>
                    <p className="text-xs text-[#1C1C2E]/50">{opt.sub}</p>
                  </div>
                </label>
              ))}
            </div>
            <div className="bg-white rounded-2xl p-6 border border-[#1C1C2E]/10">
              <label className="block text-sm font-medium text-[#1C1C2E] mb-3">Top skills (comma-separated)</label>
              <input type="text" value={skillsInput} onChange={e => setSkillsInput(e.target.value)}
                ...
                className="w-full bg-[#F8F4EE] border border-transparent rounded-xl px-4 py-3 text-[#1C1C2E] placeholder-[#1C1C2E]/30 focus:outline-none focus:border-[#0B5563] transition-colors text-sm" />
              <p className="text-xs text-[#1C1C2E]/40 mt-2">Our AI will also find hidden skills from your work history and references.</p>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-6">
            <div>
              ...
              <p className="text-[#1C1C2E]/60">We contact them directly. You don&apos;t choose what they say — that&apos;s what makes Shapi references different.</p>
            </div>
            <div className="bg-[#0B5563]/5 rounded-2xl p-5 border border-[#0B5563]/10">
              <p className="text-sm text-[#0B5563] font-medium mb-1">How it works</p>
        ...
        {step === 4 && (
          <div className="text-center space-y-6 pt-8">
            ...
            <a href="/dashboard" className="inline-block bg-[#0B5563] text-white px-8 py-4 rounded-full font-semibold text-sm hover:bg-[#094450] transition-colors">
              Go to dashboard →
            </a>
          </div>
        )}
        ...
        {step < 4 && (
          <div className="flex gap-3 mt-10">
            ...
            <button onClick={next} disabled={saving}
              className="flex-1 bg-[#0B5563] text-white py-4 rounded-full font-semibold text-sm hover:bg-[#094450] transition-colors disabled:opacity-50">
