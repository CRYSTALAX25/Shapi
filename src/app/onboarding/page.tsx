      ...
      <div className="max-w-4xl mx-auto px-6">
        <div className="h-1 bg-[rgba(255,255,255,0.08)] rounded-full">
          <div
            ...
            style={{ width: `${((step + 1) / steps.length) * 100}%`, background: 'linear-gradient(135deg, #9D8CFF, #34D399)' }}
          />
        </div>
        <div className="flex justify-between mt-2">
          {steps.map((s, i) => (
            <span key={i} className={`text-xs font-medium ${i <= step ? 'text-[#9D8CFF]' : 'text-[#5C5C6A]'}`}>
              {s}
            </span>
          ))}
        </div>
      ...
      <div className="max-w-2xl mx-auto px-6 pt-12 pb-20">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Ana Barber"
                  className="w-full bg-[#0D0C14] border border-[rgba(255,255,255,0.08)] rounded-2xl px-5 py-4 text-[#F4F4F7] placeholder-[#5C5C6A] focus:outline-none focus:border-[#9D8CFF] transition-colors" />
              </div>
              <div>
              ...
              <div>
                ...
                <textarea value={summary} onChange={e => setSummary(e.target.value)} rows={5}
                  ...
                  className="w-full bg-[#0D0C14] border border-[rgba(255,255,255,0.08)] rounded-2xl px-5 py-4 text-[#F4F4F7] placeholder-[#5C5C6A] focus:outline-none focus:border-[#9D8CFF] transition-colors resize-none" />
        ...
        {step === 1 && (
          <div className="space-y-6">
            ...
            {work.map((w, i) => (
              <div key={i} className="bg-[#0D0C14] rounded-2xl p-6 border border-[rgba(255,255,255,0.08)] space-y-4">
                <p className="text-sm font-medium text-[#9D8CFF]">{i === 0 ? 'Most recent role' : `Role ${i + 1}`}</p>
                <input type="text" placeholder="Job title" value={w.title} onChange={e => updateWork(i, 'title', e.target.value)}
                  className="w-full bg-[rgba(255,255,255,0.05)] border border-transparent rounded-xl px-4 py-3 text-[#F4F4F7] placeholder-[#5C5C6A] focus:outline-none focus:border-[#9D8CFF] transition-colors text-sm" />
                ...
                <div className="grid grid-cols-2 gap-3">
                  <input type="text" placeholder="Start (e.g. Jan 2022)" value={w.start} onChange={e => updateWork(i, 'start', e.target.value)}
                    className="bg-[rgba(255,255,255,0.05)] border border-transparent rounded-xl px-4 py-3 text-[#F4F4F7] placeholder-[#5C5C6A] focus:outline-none focus:border-[#9D8CFF] transition-colors text-sm" />
                ...
                <textarea placeholder="What did you actually achieve? Numbers, scale, impact. Don't undersell yourself." rows={4}
                  ...
                  className="w-full bg-[rgba(255,255,255,0.05)] border border-transparent rounded-xl px-4 py-3 text-[#F4F4F7] placeholder-[#5C5C6A] focus:outline-none focus:border-[#9D8CFF] transition-colors text-sm resize-none" />
              </div>
            ))}
            <button
              onClick={() => setWork(prev => [...prev, { title: '', company: '', start: '', end: '', achievements: '' }])}
              className="text-sm text-[#9D8CFF] font-medium flex items-center gap-2"
            >
              <span className="w-6 h-6 rounded-full border-2 border-[#9D8CFF] flex items-center justify-center text-xs">+</span>
              Add another role
            </button>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-6">
            ...
            <div className="bg-[#0D0C14] rounded-2xl p-6 border border-[rgba(255,255,255,0.08)] space-y-4">
              ...
              {[
                ...
                <label key={opt.val} onClick={() => setAiTier(opt.val as 'user' | 'integrator' | 'builder')}
                  ...
                  <div className={`w-5 h-5 rounded-full border-2 mt-0.5 flex-shrink-0 transition-colors ${aiTier === opt.val ? 'border-[#9D8CFF] bg-[#9D8CFF]' : 'border-[#9D8CFF]/30 group-hover:border-[#9D8CFF]'}`} />
                  <div>
            ...
            <div className="bg-[#0D0C14] rounded-2xl p-6 border border-[rgba(255,255,255,0.08)]">
        ...
        {step === 3 && (
          <div className="space-y-6">
            ...
            <div className="bg-[rgba(157, 140, 255, 0.08)] rounded-2xl p-5 border border-[rgba(157, 140, 255, 0.15)]">
              <p className="text-sm text-[#9D8CFF] font-medium mb-1">How it works</p>
        ...
        {step === 4 && (
          <div className="text-center space-y-6 pt-8">
            <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto text-white text-3xl"
              style={{ background: 'linear-gradient(135deg, #9D8CFF, #34D399)' }}>✓</div>
            <div>
            ...
            <div className="bg-[#0D0C14] rounded-2xl p-6 border border-[rgba(255,255,255,0.08)] text-left space-y-3">
              ...
              {[
                ...
                <div key={i} className="flex items-center gap-3">
                  <div className="w-5 h-5 rounded-full bg-[rgba(157, 140, 255, 0.12)] flex items-center justify-center flex-shrink-0">
                    <div className="w-2 h-2 rounded-full bg-[#9D8CFF]" />
                  </div>
