      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 pt-8 pb-20">
        ...
        <div className="gradient-border-card rounded-3xl p-8 mb-5">
          <div className="flex items-start justify-between gap-4 mb-4">
            ...
            {c.profile_live && (
              <span className="flex-shrink-0 bg-[#9D8CFF]/15 text-[#9D8CFF] text-xs font-bold px-3 py-1.5 rounded-full">
                ✓ Verified
              </span>
            )}
          </div>
          ...
          {(() => {
            ...
            return (
              <div className="mt-4 rounded-xl p-4" style={{ background: 'rgba(157, 140, 255, 0.06)', border: '1px solid rgba(157, 140, 255, 0.18)' }}>
                <p className="text-[#9D8CFF] text-[11px] font-bold uppercase tracking-wider mb-2">Looking for</p>
                <div className="flex flex-wrap gap-1.5">
                  {tRoles.map((r, i) => (
                    <span key={`r${i}`} className="text-xs font-bold px-2.5 py-1 rounded-full" style={{ background: 'rgba(157, 140, 255, 0.12)', color: '#9D8CFF' }}>{r}</span>
                  ))}
                  {tInds.map((ind, i) => (
                    <span key={`i${i}`} className="text-xs font-bold px-2.5 py-1 rounded-full" style={{ background: 'rgba(157, 140, 255, 0.12)', color: '#9D8CFF' }}>{indLabel[ind] || ind}</span>
                  ))}
                </div>
              </div>
            )
            ...
            <div className="flex flex-wrap gap-2 mt-5">
              {c.linkedin_url && (
                <a href={c.linkedin_url} target="_blank" rel="noopener noreferrer"
                  ...
                  style={{ background: 'rgba(0,119,181,0.12)', color: '#9D8CFF', border: '1px solid rgba(0,119,181,0.25)' }}>
              ...
              {c.website_url && (
                <a href={c.website_url} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold transition-all hover:opacity-80"
                  style={{ background: 'rgba(157, 140, 255, 0.08)', color: '#9D8CFF', border: '1px solid rgba(157, 140, 255, 0.2)' }}>
                  🌐 Website
                </a>
              )}
              {c.portfolio_url && (
                <a href={c.portfolio_url} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold transition-all hover:opacity-80"
                  style={{ background: 'rgba(157, 140, 255, 0.1)', color: '#9D8CFF', border: '1px solid rgba(157, 140, 255, 0.2)' }}>
                  🗂 Portfolio
                </a>
              )}
            </div>
          ...
          <div className="mt-5 pt-5 border-t border-[rgba(255,255,255,0.08)]">
            <p className="text-[#7E7E8E] text-xs">
              ...
              <a href="https://shapi.io" className="text-[#9D8CFF] hover:underline">Create a company account →</a>
            </p>
          </div>
        </div>
            ...
            {(() => {
              ...
              return (
                <div className="rounded-2xl p-6" style={{
                  background: 'linear-gradient(#0D0C14,#0D0C14) padding-box, linear-gradient(135deg,rgba(157, 140, 255, 0.4),rgba(157, 140, 255, 0.3)) border-box',
                  border: '1px solid transparent',
                  boxShadow: '0 1px 2px rgba(0,0,0,0.45), 0 16px 40px rgba(0,0,0,0.35)',
                }}>
                  <div className="flex items-center gap-2.5 mb-3">
                    <span className="w-1.5 h-6 rounded-full" style={{ background: '#9D8CFF' }} />
                    <h2 className="text-[#F4F4F7] font-black text-xl tracking-tight">AI Cross-Check</h2>
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(157, 140, 255, 0.15)', color: '#9D8CFF' }}>✓ verified across references</span>
                  </div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4 ml-4">
                    {(report.claims_verified?.length ?? 0) > 0 && (
                      <div>
                        <p className="text-[#9D8CFF] text-[11px] font-bold uppercase tracking-wider mb-2">✓ Independently confirmed</p>
                    ...
                    {(report.top_skills?.length ?? 0) > 0 && (
                      <div>
                        <p className="text-[#9D8CFF] text-[11px] font-bold uppercase tracking-wider mb-2">Most-cited strengths</p>
                        <div className="flex flex-wrap gap-1.5">
                          {report.top_skills!.map((s, i) => (
                            <span key={i} className="text-[11px] font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(157, 140, 255, 0.1)', color: '#9D8CFF' }}>{s}</span>
                          ))}
                        </div>
                      </div>
                    )}
              ...
              <div className="gradient-border-card rounded-2xl p-6">
                ...
                <div className="space-y-3">
                  {userAnswers.map((m: {role: string; content: string}, i: number) => (
                    <div key={i} className="rounded-xl px-4 py-3 border-l-2 border-[#9D8CFF]/40" style={{ background: 'rgba(255,255,255,0.05)' }}>
                ...
                {langProficiency?.cefr_level && (
                  <div className="border-t border-[rgba(255,255,255,0.08)] pt-4 mt-2">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      <div className="bg-[#9D8CFF]/10 border border-[#9D8CFF]/20 rounded-xl px-3 py-2 text-center flex-1 min-w-[70px]">
                        <p className="text-[#9D8CFF] text-base font-black">{langProficiency.cefr_level}</p>
                        <p className="text-[#7E7E8E] text-[10px]">CEFR</p>
                      </div>
                      {langProficiency.ielts_equivalent && (
                        <div className="bg-[#9D8CFF]/10 border border-[#9D8CFF]/20 rounded-xl px-3 py-2 text-center flex-1 min-w-[70px]">
                          <p className="text-[#9D8CFF] text-base font-black">{langProficiency.ielts_equivalent}</p>
                          <p className="text-[#7E7E8E] text-[10px]">IELTS est.</p>
                        </div>
                      )}
                      {langProficiency.english_level && langProficiency.english_level !== 'unassessed' && (
                        <div className="bg-[#9D8CFF]/10 border border-[#9D8CFF]/20 rounded-xl px-3 py-2 text-center flex-1 min-w-[70px]">
                          <p className="text-[#9D8CFF] text-base font-black">{langProficiency.english_level}</p>
              ...
              return (
                <div className="gradient-border-card rounded-2xl p-6">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(157, 140, 255, 0.12)', color: '#9D8CFF' }}>LIVE</span>
                  </div>
              ...
              return (
                <div className="gradient-border-card rounded-2xl p-6">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full" style={{ background: 'rgba(157, 140, 255, 0.12)', color: '#9D8CFF' }}>◆ self-assessed</span>
                  </div>
                  <div className="space-y-3">
                    {dims.map(d => {
                    ...
                    return (
                      <div key={i} className="flex items-center justify-between gap-2 rounded-lg px-3 py-2" style={{ background: 'rgba(255,255,255,0.05)' }}>
                        ...
                        <span className="flex items-center gap-1.5 flex-shrink-0">
                          {cc.status === 'in_progress' && <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full" style={{ background: 'rgba(157, 140, 255, 0.12)', color: '#9D8CFF' }}>● In progress</span>}
                          ...
                          {cc.credential_url && <a href={cc.credential_url as string} target="_blank" rel="noopener noreferrer" className="text-[#9D8CFF] text-[10px] font-bold">cert ↗</a>}
                        </span>
                      </div>
                    )
                  })}
            ...
            {refCount > 0 && (
              <div className="rounded-2xl p-6" style={{
                background: refCount >= 3
                  ? 'linear-gradient(#0D0C14,#0D0C14) padding-box, linear-gradient(135deg,rgba(157, 140, 255, 0.35),rgba(157, 140, 255, 0.25)) border-box'
                  : 'linear-gradient(#0D0C14,#0D0C14) padding-box, linear-gradient(135deg,rgba(157, 140, 255, 0.15),rgba(157, 140, 255, 0.15)) border-box',
                border: '1px solid transparent',
                boxShadow: '0 1px 2px rgba(0,0,0,0.45), 0 16px 40px rgba(0,0,0,0.35)',
              }}>
                ...
                <div className="space-y-1.5 mb-4">
                  {managerRefs > 0 && (
                    <div className="flex items-center gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-[#9D8CFF]/60 flex-shrink-0" />
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {refSkills.map((s, i) => (
                        <span key={i} className="text-[10px] font-bold px-2 py-0.5 rounded-full"
                          style={{ background: 'rgba(157, 140, 255, 0.1)', color: '#9D8CFF' }}>
                          {s}
                        </span>
                      ))}
                    </div>
