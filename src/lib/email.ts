...
export async function sendProfileLiveEmail(to: string, name: string, profileId: string) {
  ...
  const html = emailShell(`
    ...
    <div style="background:rgba(56, 189, 248, 0.06);border:1px solid rgba(56, 189, 248, 0.12);border-radius:10px;padding:14px 18px;margin:20px 0">
      ...
      <a href="${profileUrl}" style="color:#38BDF8;font-size:14px;font-weight:700;text-decoration:none;word-break:break-all">${profileUrl}</a>
    </div>
...
export async function sendCvKitEmail(to: string, name: string, tier: 'kit' | 'pro' = 'kit') {
  ...
  if (tier === 'pro') {
    const html = emailShell(`
      ...
      ${p(`<strong style="color:#38BDF8">Plus what Pro ($59) adds:</strong><br>
...
export async function sendNominatedReferenceEmail(opts: {
  ...
  const html = emailShell(`
    ...
    <div style="background:rgba(56, 189, 248, 0.08);border:1px solid rgba(56, 189, 248, 0.2);border-radius:10px;padding:14px 18px;margin:16px 0">
      <p style="color:rgba(56, 189, 248, 0.8);font-size:13px;font-weight:700;margin:0">${candidateFirst} doesn&apos;t know we&apos;ve reached out — you can be completely candid.</p>
    </div>
...
export async function sendCompanyMatchEmail(
  ...
  const html = emailShell(`
    ...
    ${p(`A verified candidate just completed their profile and scored <strong style="color:#38BDF8">${matchScore}%</strong> against your <strong style="color:rgba(255,255,255,0.8)">${roleName}</strong> role — that puts them in the <em>${label} match</em> category.`)}
...
export async function sendWelcomeEmail({ to, name }: { to: string; name: string }) {
  ...
  const html = emailShell(`
    ...
    <a href="${SITE}/dashboard" style="color:#38BDF8;font-size:14px;font-weight:700;text-decoration:none">Go to your dashboard →</a>
    ${signoff()}
  `)
...
export async function sendConciergeOutreach(opts: {
  ...
  const html = emailShell(`
    ...
    <div style="background:rgba(56, 189, 248, 0.06);border:1px solid rgba(56, 189, 248, 0.18);border-radius:10px;padding:14px 18px;margin:0 0 20px">
      <p style="color:rgba(56, 189, 248, 0.85);font-size:12px;font-weight:700;margin:0">Reached out via Shapi — profile independently verified</p>
    </div>
...
export async function sendConciergeNudge(opts: {
  ...
  const html = emailShell(`
    ...
    ${p(`Your Shapi Concierge scanned today&apos;s openings and drafted personalised intros for <strong style="color:#38BDF8">${count} ${roleWord}</strong> that fit your profile. Each one&apos;s ready — review and approve before it goes out.`)}
...
export async function sendConciergeReplyAlert(opts: {
  ...
  const html = emailShell(`
    ...
    ${safeReply ? `
    <div style="background:rgba(56, 189, 248, 0.06);border:1px solid rgba(56, 189, 248, 0.18);border-radius:10px;padding:14px 18px;margin:0 0 20px">
      <p style="color:rgba(56, 189, 248, 0.85);font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;margin:0 0 8px">What they said</p>
...
export async function sendCompanyWelcomeEmail(opts: {
  ...
  const html = emailShell(`
    ...
    ${divider()}
    ${p(`Receipt + invoice come from Stripe directly. Manage billing or cancel from your <a href="${SITE}/api/stripe/portal" style="color:#38BDF8">customer portal</a>.`)}
...
export async function sendLifecycleCommsEmail(opts: {
  ...
  const html = emailShell(`
    <div style="background:rgba(56, 189, 248, 0.06);border:1px solid rgba(56, 189, 248, 0.18);border-radius:10px;padding:14px 18px;margin:0 0 20px">
      <p style="color:rgba(56, 189, 248, 0.85);font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;margin:0">${programLabel}</p>
    </div>
    <p style="color:rgba(255,255,255,0.7);font-size:15px;line-height:1.7;margin:0 0 16px">${safeBody}</p>
    ${divider()}
